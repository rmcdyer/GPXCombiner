...
public class Combiner {
    ...
    private static String fileActivityType = "";
    private static String fileFooter = "  </trkseg>\n </trk>\n</gpx>";

    private static String gpxFileExtension = ".gpx";
    ...
    public static void setActivityType(String s) {
        fileActivityType = s;
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (stravaData.size() == 0) {
            try {
                ...
                if (!stravaFileName.contains(gpxFileExtension)) {
                    stravaFileName = stravaFileName.concat(gpxFileExtension);
                }
            } catch (Exception e) {
                ;
            }
        ...
        while (fitbitData.size() == 0) {
            try {
                ...
                if (!fitbitFileName.contains(gpxFileExtension)) {
                    fitbitFileName = fitbitFileName.concat(gpxFileExtension);
                }
            } catch (Exception e) {
                ;
            }
...
//                    t.setCadence(stravaData.get(j).getCadence());
                    stravaData.set(i, t);
                }
            }
    ...
    static ArrayList<TrkSeg> readFile(String fileName, boolean isStravaFile) {
        ...
        try {
            ...
            if (isStravaFile) {
                ...
                sb = new StringBuilder();
                line = br.readLine(); //<type> $type </type>
                sb.append(line);
                sb.append(System.lineSeparator());
                Combiner.setActivityType(sb.toString());
                sb = new StringBuilder();
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (line.contains("</trkpt>")) {
                    ...
                    System.out.println(trkptCount + " datapoints processed");
                }
            }
...
class GPXWriter {
    static void writeFile(ArrayList<TrkSeg> list) {
        try {
            ...
            writer.print(Combiner.getActivityType());
            writer.print("<trkseg>\n");
            writer.flush();
            ...
            writer.flush();
            System.out.println("trkSegCount is " + trkSegCount);
...
class TrkSeg implements Comparable<TrkSeg> {
    ...
    public void setCadence(int cadParam) {
        cad = cadParam;
    }
    ...
    TrkSeg(String s) {
        ...
        if (s.indexOf("gpxtpx:cad") >= 0) {
            s = s.substring(s.indexOf("gpxtpx:cad"));
            s = s.substring(s.indexOf(">") + 1);
            cad = Integer.parseInt(s.substring(0, s.indexOf("<")));
        } else {
            cad = 0;
        }
    }
    ...
    public String toString() {
        ...
        if (hr > 0 || cad > -1) {
            ...
            if (cad > -1) {
                sb.append("        <gpxtpx:cad>" + cad + "</gpxtpx:cad>\n");
            }
            sb.append("      </gpxtpx:TrackPointExtension>\n");
            sb.append("    </extensions>\n");
        }
